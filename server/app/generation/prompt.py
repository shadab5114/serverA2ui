"""build_system_prompt: turns the LLM into an A2UI v0.9 generator (reference: src/systemPrompt.js).

The FULL component schemas (bundled with the $defs they reference) are embedded
so the model uses only real components, real property names and the correct
property shapes. The template text was extracted verbatim from the Node
implementation (src/systemPrompt.js, now in git history) and proven byte-identical
before Node was retired; tests/test_prompt_parity.py keeps it as a snapshot.
"""
...
_TEMPLATE = """You are an A2UI (Agent-to-UI) generation engine that emits A2UI **v0.9**.
...
- catalogId: required, MUST be exactly "<<CATALOG_ID>>".
...
def schema_json(value: Any) -> str:
    """Minified JSON, byte-compatible with JS JSON.stringify for catalog data."""
    return json.dumps(value, ensure_ascii=False, separators=(",", ":"))


def build_system_prompt(catalog: dict[str, Any]) -> str:
    """catalog: {catalogId, components, defs} (see grounding.catalog)."""
    # Minified to keep the prompt small; the model parses compact JSON fine.
    schema_block = schema_json({"components": catalog["components"], "$defs": catalog["defs"]})
    catalog_id = catalog.get("catalogId") or "<catalogId from the catalog>"
    return _TEMPLATE.replace("<<CATALOG_ID>>", catalog_id).replace("<<SCHEMA_BLOCK>>", schema_block)
